...
def integrate_any(v_st, v_params, controls):
    x_update = v_st.x + np.cos(v_st.theta) * v_st.v * v_params.step
    y_update = v_st.y + np.sin(v_st.theta) * v_st.v * v_params.step
    theta_update = v_st.theta + (np.tan(v_params.steerMap * controls[1]) / v_params.l) * v_st.v * v_params.step
    ...
    ta = f1 - f0
    ta = 0 if (abs(v_st.v - 0) < 1e-9) and (ta < 0) else ta
